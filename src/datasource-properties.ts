...
export async function propertiesSchemaForDatasource(client: Client, data_source_id: string) {
  ...
  return propertiesSchemaForDatasourceProperties(dataSource.properties);
}

export function propertiesSchemaForDatasourceProperties(properties: Record<string, DataSourcePropertyConfigResponse>) {
  ...
  const schema = Object.fromEntries(
    Object.entries(properties).map(([key, value]: [string, DataSourcePropertyConfigResponse]) => {
